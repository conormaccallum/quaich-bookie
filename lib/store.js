...
async function getState() {
  ...
  const [bettorsRows, marketsRows, outcomesRows, betsRows] = await Promise.all([
    ...
    sql`
      select id, name, category, sort_order, is_active, margin, default_stake, auto_balance, rebalance_sensitivity, created_at
      from markets
      order by category asc, sort_order asc, created_at desc, name asc
    `,
    sql`
      select id, market_id, name, team, true_probability, base_offered_decimal, position_index
      from outcomes
  ...
  const markets = marketsRows.map((market) => ({
    ...
    sortOrder: Number(market.sort_order),
...
async function createMarket(body) {
  ...
  await sql.begin(async (tx) => {
    await tx`
      insert into markets (id, name, category, sort_order, is_active, margin, default_stake, auto_balance, rebalance_sensitivity, created_at)
      values (
        ${market.id},
        ${market.name},
        ${market.category},
    ...
    for (const outcome of market.outcomes) {
      await tx`
        insert into outcomes (id, market_id, name, team, true_probability, base_offered_decimal, position_index)
        values (
          ${outcome.id},
          ${market.id},
          ${outcome.name},
