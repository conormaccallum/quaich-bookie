...
async function getState() {
  ...
  const [bettorsRows, marketsRows, outcomesRows, betsRows] = await Promise.all([
    ...
    sql`
      select id, name, category, margin, default_stake, auto_balance, rebalance_sensitivity, created_at
      from markets
...
async function createMarket(body) {
  ...
  await sql.begin(async (tx) => {
    await tx`
      insert into markets (id, name, category, margin, default_stake, auto_balance, rebalance_sensitivity, created_at)
      values (
        ${market.id},
        ${market.name},
        ${market.category},
