...
const MARKET_CATEGORIES = ["Outright", "Match Bets", "Individual", "Props"];
...
function buildMarket(body, marketId = crypto.randomUUID()) {
  ...
  const sortOrder = clampNumber(body.sortOrder, -100000, 100000, 0);
  ...
  const validated = rawOutcomes
    .map((outcome, index) => ({
      ...
      team: sanitizeOutcomeTeam(outcome.team),
  ...
  return {
    ...
    markets: Array.isArray(store.markets)
      ? store.markets.map((market) => ({
          ...market,
          category: sanitizeCategory(market.category),
          sortOrder: clampNumber(market.sortOrder, -100000, 100000, 0),
          autoBalance: market.autoBalance !== false,
          rebalanceSensitivity: clampNumber(market.rebalanceSensitivity, 0, 5, 0.12),
...
function sanitizeCategory(value) {
  return MARKET_CATEGORIES.includes(value) ? value : "Outright";
}

function sanitizeOutcomeTeam(value) {
  return ["Scotland", "USA", "None"].includes(value) ? value : "None";
}
